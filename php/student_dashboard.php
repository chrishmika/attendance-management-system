...
if (!isset($_SESSION['std_id'])) {
    ...
    $userId = $_SESSION['user_id']; // Ensure user_id is stored in the session upon login
    
    // Fetch `std_id` for the logged-in user from the `uoj_student` table
    $query = "SELECT std_id FROM uoj_student WHERE user_id = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        $_SESSION['std_id'] = $row['std_id']; // Store std_id in session
    } else {
...
// Now you can use `$_SESSION['std_id']` anywhere in this script
...
//$queryx = "SELECT std_id FROM uoj_student WHERE uoj_user.user_id = $isLoggedIn";
//$result1 = mysqli_query($conn, $queryx);
